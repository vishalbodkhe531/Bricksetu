...
export const InventoryView: React.FC = () => {
  ...
  const handleAdjustmentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      loadInventoryData();
    } catch (err: any) { setAdjustError(err.message || 'Failed to submit stock adjustment'); }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button onClick={() => { setAdjustError(''); setIsAdjustOpen(true); }} className="bg-orange-500 hover:bg-orange-600 text-white font-bold gap-2 h-10 px-4 shadow-md shadow-orange-500/20 text-xs sm:text-sm border-0 cursor-pointer">
      ...
      <Tabs defaultValue="lots" className="space-y-4">
        <TabsList className="bg-muted p-1 rounded-xl">
          <TabsTrigger value="lots" className="gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg cursor-pointer">
            <Layers className="size-3.5" /> Stock Lots ({stockLots.length})
          </TabsTrigger>
          <TabsTrigger value="ledger" className="gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg cursor-pointer">
                ...
                <TableBody>
                  {ledger.length === 0 ? (
                    ...
                    <TableRow key={item.id} className="border-border hover:bg-muted/40">
                      ...
                      <TableCell className="text-foreground text-sm font-medium">{item.brick_type_name} ({item.brick_grade_name})</TableCell>
        ...
        <DialogContent className="bg-card border-border text-card-foreground sm:max-w-[480px]">
          ...
          <form onSubmit={handleAdjustmentSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-muted-foreground text-xs font-semibold">Adjustment Type</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-muted-foreground text-xs font-semibold">Brick Grade</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-muted-foreground text-xs font-semibold">Quantity (Bricks)</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-muted-foreground text-xs font-semibold">Reason / Notes</Label>
            ...
            <div className="flex justify-end gap-2.5 pt-3 border-t border-border">
              ...
              <Button type="submit" className="bg-orange-500 hover:bg-orange-600 text-white font-semibold border-0 cursor-pointer">Commit Adjustment</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
