...
export function Header({ user, onMobileMenuToggle }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-card/95 backdrop-blur px-4 sm:px-6 lg:px-2">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="hidden sm:flex items-center gap-2.5 rounded-full border border-border bg-card px-2.5 py-1 shadow-xs">
          ...
          <Badge
            variant="outline"
            className="text-[10px] font-bold tracking-wider py-0 px-1.5 uppercase bg-muted/50 border-border text-muted-foreground"
          >
            {user?.role || "ADMIN"}
          </Badge>
        </div>
